...
module.exports = function (api) {
	...
	api.loadSource(async actions => {
		...
		const { data } = await axios.get('https://carrotquest.cdn.prismic.io/api/v2')
			.then( response => {
				axios.get('https://carrotquest.cdn.prismic.io/api/v2/documents/search?ref=' + response.data.refs[0].ref + '&q=%5B%5B%3Ad+%3D+at%28document.id%2C+%22X0ZSlRIAAPalRttu%22%29+%5D%5D' )
					.then( response => {
						return response.data.results[0].data.body[0].items.reverse()
					})
			})
		console.log(data)
